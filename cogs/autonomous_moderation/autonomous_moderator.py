            ...
            for v in report.violations:
                ...
                if messages_to_delete:
                    ...
                    if len(author_ids) == 1:
                        ...
                        # Multiple authors: fan out per author to avoid misattribution
                        ...
                        for author_id in author_ids:
                            per_author_messages = [m for m in messages_to_delete if m.author.id == author_id]
                            if not per_author_messages:
                                continue
                            ...
                            await strike.perform_disciplinary_action(
                                ...
                                reason=reason,
                                source="batch_ai",
                                message=per_author_messages
                            )

                            violation_cache[author_id].append((rule, ", ".join(configured)))
                            embed = discord.Embed(
                                title="AI-Flagged Violation",
                                description=(
                                    f"User: {member.mention if member else author_id}\n"
                                    f"Rule Broken: {rule}\n"
                                    f"Reason: {reason}\n"
                            ...
                            if aimod_debug:
                                # Indicate that we fanned out due to mixed authors
                                embed.add_field(name="Fan-out", value="Multiple authors detected; applied per-author actions.", inline=False)
                                try:
                                ...
                                def _trim2(s: str, n: int = 300) -> str:
                                    s = s or ""
                                    return s if len(s) <= n else s[:n] + "�?�"
                                flagged_lines = []
                                for m in per_author_messages:
                                    content = m.content or "[no text content]"
                                    flagged_lines.append(f"�?� ID {m.id}: {_trim2(content)}")
                                flagged_blob = "\n".join(flagged_lines)
                        ...
                        # Done handling mixed authors; proceed to next violation
                        continue
