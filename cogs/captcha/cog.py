...
from .config import (
    ...
    resolve_api_base,
    resolve_public_verify_url,
)
from .delivery import CaptchaDeliveryMixin
from .embed_sync import CaptchaEmbedSyncMixin
from .utils import coerce_grace_period, coerce_positive_int, format_duration

_logger = logging.getLogger(__name__)

class CaptchaCog(CaptchaDeliveryMixin, CaptchaEmbedSyncMixin, commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member) -> None:
        ...
        embed_channel_id = coerce_positive_int(settings.get("captcha-embed-channel-id"))
        grace_setting = coerce_grace_period(settings.get("captcha-grace-period"))
        ...
        grace_display = grace_setting or format_duration(grace_delta)
        max_attempts = coerce_positive_int(settings.get("captcha-max-attempts"))
    ...
    def _build_public_verification_url(self, guild_id: int) -> str:
        return build_public_verification_url(self._public_verify_url, guild_id)
